...
export default function Vehicules() {
    ...
    React.useEffect(() => {
        axios.get('http://localhost:9092/query/vehicule/')
        .then(res => {
    ...
    if(vehicules.length === 0 && Loading===true) return (
  ...
  return (
    <div>
        <div className="flex justify-between items-center mt-10">
            <h1 className="text-3xl mx-12 md:text-6xl font-extrabold leading-tighter tracking-tighter mb-4">Vehicules</h1>
            <Link to="/vehicules/addVehicule">
        ...
        <table className="table-auto mx-auto">
            <thead>
                <tr>
                    ...
                    <th className="px-4 py-2">Puissance Fiscale</th>
                    <th className="px-4 py-2">Actions</th>
                </tr>
            </thead>
            <tbody>
                {vehicules.map(vehicule => (
                    <tr key={vehicule.matricule}>
                        <td className="border px-4 py-2">{vehicule.matricule}</td>
                        <td className="border px-4 py-2">{vehicule.marque}</td>
                        <td className="border px-4 py-2">{vehicule.modele}</td>
                        <td className="border px-4 py-2">{vehicule.puissance}</td>
